 ...
 * @version    SVN: $Id: BasesfGuardAuthActions.class.php,v 1.1.1.1 2012/03/24 12:16:32 nitin Exp $
 */
  ...
  public function executeSignin($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        if($asUserRoleId['group_id'] != sfConfig::get('app_cemrole_normaluser'))
        {
            $values = $this->form->getValues();
            $this->getUser()->signin($values['user'], array_key_exists('remember', $values) ? $values['remember'] : false);            
            // SET USERNAME AND ENCODED PASSWORD INTO SESTION FOR LOGIN INTO GROUP OFFICE.		
            if( $this->getUser()->isSuperAdmin() || $this->getUser()->getAttribute('groupid') == sfConfig::get('app_cemrole_manager') || $this->getUser()->getAttribute('groupid') == sfConfig::get('app_cemrole_staffadmin') || $this->getUser()->getAttribute('groupid') == sfConfig::get('app_cemrole_staff') )
            {
