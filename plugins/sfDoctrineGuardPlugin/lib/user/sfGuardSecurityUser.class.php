 ...
 * @version    SVN: $Id: sfGuardSecurityUser.class.php,v 1.1.1.1 2012/03/24 12:16:34 nitin Exp $
 */
class sfGuardSecurityUser extends sfBasicSecurityUser
{
  ...
  public function signIn($user, $remember = false, $con = null)
  {
    ...
    $this->setAttribute('firstname', $user->getFirstName(), 'sfGuardSecurityUser');
    $this->setAttribute('lastname', $user->getLastName(), 'sfGuardSecurityUser');
    $this->setAttribute('email_address', $user->getEmailAddress(), 'sfGuardSecurityUser');
    $this->setAttribute('issuperadmin', $user->getIsSuperAdmin());
    $this->setAttribute('userid', $user->getId());
    
		$asUserRoleId = Doctrine_Query::create()
						->select('c.group_id')
						->from('UserCemetery c')
			...
			if($asUserRoleId) 
			{			
				$this->setAttribute('groupid', $asUserRoleId->getGroupId());
				$this->setAttribute('cemeteryid', $asUserRoleId->getCemCemeteryId());
				
				// FOR set country id into session of cemetry.
				$oCemetery = Doctrine::getTable('CemCemetery')->find($asUserRoleId->getCemCemeteryId());
				$snCountryId = ($oCemetery) ? $oCemetery->getCountryId() : '';
				$this->setAttribute('cem_country_id', $snCountryId);

				$this->setAttribute('title', $asUserRoleId->getTitle());
			}else {
				$this->setAttribute('groupid', '');
				$this->setAttribute('cemeteryid', '');
				$this->setAttribute('cem_country_id', $snCountryId);
			}
			
    $this->setAttribute('userroleid', $user->getIsSuperAdmin());
	...
	if($asUserRoleId) {	
		if($asUserRoleId->getGroupId() == 2) {
			$this->addCredentials('cemadmin');
		}
		
		if($asUserRoleId->getGroupId() == 3) {
			$this->addCredentials('cemstaff');
		}
		
		if($asUserRoleId->getGroupId() == 4) {
			$this->addCredentials('cemstaffadmin');
		}				
		
		if($asUserRoleId->getGroupId() == 5) {
			$this->addCredentials('fnd');
		}		
		
		if($asUserRoleId->getGroupId() == 6) {
			$this->addCredentials('stonemason');
		}				
	}
