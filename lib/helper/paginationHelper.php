 ...
 *
 */
function pager_navigation($pager, $snPaging, $ssForm, $snPaggingDropDown, $IsPlugin)
{  
    if ($pager->getNbResults() > 0)
    {
        ...
        {  
            ...
            if ($pager->getPage() != 1)
            {
                $ssNavigation .= '<li>'.link_to_function(image_tag('first.png',array('title'=>__('First'),'name'=>'right')),'setFirstPage('.$pager->getFirstPage().',\''.$ssForm.'\')', array('id' => 'first')).'</li>';            
                $ssNavigation .= '<li>'.link_to_function(image_tag('prev.png',array('title'=>__('Previous'),'name'=>'right')),'setPreviousPage(\''.$ssForm.'\')', array('id' => 'previous')).'</li>';
            }
            else
            {
                $ssNavigation .= '<li>'.image_tag('first-disable.png',array('title'=>__('First'),'name'=>'right')).'</li>';         
                $ssNavigation .= '<li>'.image_tag('prev-disable.png',array('title'=>__('Previous'),'name'=>'right')).'</li>';
            }
            ...
            {
                $ssNavigation .= '<li>'.link_to_function(image_tag('next.png',array('title'=>__('Next'))),'setNextPage(\''.$ssForm.'\')', array('id' => 'next')).'</li>';
                $ssNavigation .= '<li>'.link_to_function(image_tag('last.png',array('title'=>__('Last'))),'setLastPage('.$pager->getLastPage().',\''.$ssForm.'\')', array('id' => 'last')).'</li>';            
