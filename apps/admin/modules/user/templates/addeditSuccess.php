...
<div id="wapper">
    ...
    <h1><?php echo $sf_params->get('id') ?  __('Edit User') : __('Add New User');?></h1>
    <div id="message">
        <div id="success_msgs">
                ...
                <div class="left_part">
                    <div id="info">
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                            ...
                            <?php foreach($oSfGuardUsersForm as $ssKey => $asField):
                                
                                if($ssKey == 'group_id'):
                                if(!$sf_user->isSuperAdmin()) :?>
                                    <tr>
                                        <td valign="middle" align="right" width="20%">
                                            <?php echo __('Select Award')?>
                                        </td>			
                                        <td valign="middle" width="80%">
                                            <?php include_partial('get_award_list', array('asAwards' => $asAwards,'snCementeryId' => $snCementeryId)); 
                                            ?>
                                        </td>
                                    </tr>
                                <?php
                                ...
                                if($sf_user->isSuperAdmin()) :
                                    if($ssKey == 'cem_country_id' || $ssKey == 'cem_cemetery_id'):?>
									<tr>
										<td valign="middle" align="right" width="20%">
...
<?php 
    ...
    echo javascript_tag("
		...
		{
			var snCountryId = jQuery('#sf_guard_user_cem_country_id').val();
			var snCemeteryId = $('#user_cem_cemetery_id option').length;
			if(snCountryId > 0 && snCemeteryId == 1)
				callAjaxRequest(snCountryId,'".url_for('user/getCementryListAsPerCountry')."','user_cementery_list');
		});
