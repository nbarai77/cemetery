...
<div id="wapper">    
    ...
    <div id="result"></div>
    <h1>
        <?php echo __('Grantee Log list');?> 
        <span id="no_of_items"><?php include_partial('global/total_items',array('snTotalItems'=> $snPageTotalGranteeLogPages));?></span>
    ...
    <?php
        echo jq_form_remote_tag(
                array(
                    'url'       => url_for($ssModuleName.'/granteeLog?request_type=ajax_request'),
		...
		echo input_hidden_tag('logtype','grantee', array('readonly' => true));
        ...
        echo '<div>';
            echo '<div style="width:80%;float:left;">';    
                    ...
                    echo '<div  id="contentlisting" >';
                        include_partial(
                            'grantee_log_list_middle_part',
                            array(
                                'oGranteeLogList'   => $oGranteeLogList,
                                'amGranteeLogList'  => $amGranteeLogList,
                    ...
                    include_partial(
                        ...
                        array(
                            'amPagerSearchResults'  => $oGranteeLogList, 
                            'amExtraParameters'     => $amExtraParameters
                        )
                    );
...
<?php 
	...
	echo javascript_tag('
		...
		function savePDF()
		{
			...
			ssURL = ssURL+"?logtype=granteelog&cemetery_id="+jQuery("#searchCemId").val()+"&user_id="+jQuery("#searchUserId").val()+"&operation_date="+jQuery("#searchOperationDate").val();
