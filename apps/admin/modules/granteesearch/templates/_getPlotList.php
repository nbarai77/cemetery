	...
	<?php echo select_tag('grantee_ar_plot_id', options_for_select($asPlotList , isset($snPlotId) ? $snPlotId : '', 'include_custom='.__('Select Plot')),
				array('tabindex'=>6,'onChange' => jq_remote_function(
							array('url'		=> url_for('granteesearch/getGraveListAsPerPlot'),
								  ...
								  'with'	=> "'plot_id='+this.value+'&country_id='+$('#grantee_country_id').val()+'&cemetery_id='+$('#grantee_cem_cemetery_id').val()+'&area_id='+$('#grantee_ar_area_id').val()+'&section_id='+$('#grantee_ar_section_id').val()+'&row_id='+$('#grantee_ar_row_id').val()",
