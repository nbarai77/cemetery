 ...
 *
 * @package    Cemetery
 * @subpackage Denomination
 * @author     Nitin Barai
 * @author      
 *
 */
sfProjectConfiguration::getActive()->loadHelpers(array('I18N'));
class denominationActions extends sfActions
{
    /**
     * preExecutes  action
     *
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_denomination';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('Denomination', $request->getParameter('id'),'id');
            $this->getUser()->setFlash('snSuccessMsgKey', 3);      
        }

        // Get cms page list for listing.
        $oGuardGroupPageListQuery = Doctrine::getTable('Denomination')->getDenominationList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('Denomination', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listDenominationUpdate');
    }
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('Denomination')->find($snIdGuardGroup));
            $this->oDenominationForm = new DenominationForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oDenominationForm = new DenominationForm();
		}

        $this->getConfigurationFields($this->oDenominationForm);

        if($oRequest->isMethod('post'))
        {
            $this->oDenominationForm->bind($oRequest->getParameter($this->oDenominationForm->getName()));

            if($this->oDenominationForm->isValid())
            {
                $snIdGroup = $this->oDenominationForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('denomination/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('denomination/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
