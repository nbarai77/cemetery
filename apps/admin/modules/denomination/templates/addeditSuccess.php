...
<div id="wapper">
<?php 
    echo $oDenominationForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oDenominationForm->getObject()->isNew() ? '?id='.$oDenominationForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oDenominationForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oDenominationForm->getObject()->isNew() ?  __('Add New Denomination') : __('Edit Denomination');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oDenominationForm['name']->renderLabel($oDenominationForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oDenominationForm['name']->hasError()):
                                    	    echo $oDenominationForm['name']->renderError();
                                        endif;
									    echo $oDenominationForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oDenominationForm['is_enabled']->renderLabel($oDenominationForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oDenominationForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oDenominationForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
