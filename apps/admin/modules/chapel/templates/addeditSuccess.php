...
<div id="wapper">
<?php 
    echo $oCemChapelForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oCemChapelForm->getObject()->isNew() ? '?id='.$oCemChapelForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oCemChapelForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oCemChapelForm->getObject()->isNew() ?  __('Add New Chapel Type') : __('Edit Chapel Type');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		...
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oCemChapelForm['country_id']->renderLabel($oCemChapelForm['country_id']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oCemChapelForm['country_id']->hasError()):
                                    	    echo $oCemChapelForm['country_id']->renderError();
                                        endif;
									    echo $oCemChapelForm['country_id']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>							
							<tr>
							...
							<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oCemChapelForm['name']->renderLabel($oCemChapelForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oCemChapelForm['name']->hasError()):
                                    	    echo $oCemChapelForm['name']->renderError();
                                        endif;
									    echo $oCemChapelForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
							<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oCemChapelForm['description']->renderLabel($oCemChapelForm['description']->renderLabelName() );?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oCemChapelForm['description']->hasError()):
                                    	    echo $oCemChapelForm['description']->renderError();
                                        endif;
									    echo $oCemChapelForm['description']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>							
                    		<tr>
    ...
    <?php
        ...
        echo $oCemChapelForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
