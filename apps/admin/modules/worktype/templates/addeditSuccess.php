...
<div id="wapper">
<?php 
    echo $oCemWorktypeStonemasonForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oCemWorktypeStonemasonForm->getObject()->isNew() ? '?id='.$oCemWorktypeStonemasonForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oCemWorktypeStonemasonForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oCemWorktypeStonemasonForm->getObject()->isNew() ?  __('Add New Work Type Stone Mason') : __('Edit Work Type Stone Mason');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oCemWorktypeStonemasonForm['name']->renderLabel($oCemWorktypeStonemasonForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oCemWorktypeStonemasonForm['name']->hasError()):
                                    	    echo $oCemWorktypeStonemasonForm['name']->renderError();
                                        endif;
									    echo $oCemWorktypeStonemasonForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oCemWorktypeStonemasonForm['is_enabled']->renderLabel($oCemWorktypeStonemasonForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oCemWorktypeStonemasonForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oCemWorktypeStonemasonForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
