<?php
/**
 * CemWorktypeStonemason actions.
 *
 * @package    Cemetery
 * @subpackage CemWorktypeStonemason
 * @author     Nitin Barai
 * @author      
 *
 */
sfProjectConfiguration::getActive()->loadHelpers(array('I18N'));
class worktypeActions extends sfActions
{
    /**
     * preExecutes  action
     *
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_work_type';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('CemWorktypeStonemason', $request->getParameter('id'),'id');
            $this->getUser()->setFlash('snSuccessMsgKey', 3);      
        }

        // Get cms page list for listing.
        $oGuardGroupPageListQuery = Doctrine::getTable('CemWorktypeStonemason')->getCemWorktypeStonemasonList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('CemWorktypeStonemason', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listCemWorktypeStonemasonUpdate');
    }
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('CemWorktypeStonemason')->find($snIdGuardGroup));
            $this->oCemWorktypeStonemasonForm = new CemWorktypeStonemasonForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oCemWorktypeStonemasonForm = new CemWorktypeStonemasonForm();
		}

        $this->getConfigurationFields($this->oCemWorktypeStonemasonForm);

        if($oRequest->isMethod('post'))
        {
            $this->oCemWorktypeStonemasonForm->bind($oRequest->getParameter($this->oCemWorktypeStonemasonForm->getName()));

            if($this->oCemWorktypeStonemasonForm->isValid())
            {
                $snIdGroup = $this->oCemWorktypeStonemasonForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('worktype/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('worktype/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
