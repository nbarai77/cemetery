...
<div id="wapper">
<?php 
    echo $oServiceTypeForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oServiceTypeForm->getObject()->isNew() ? '?id='.$oServiceTypeForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oServiceTypeForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oServiceTypeForm->getObject()->isNew() ?  __('Add New Service Type') : __('Edit Service Type');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oServiceTypeForm['name']->renderLabel($oServiceTypeForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oServiceTypeForm['name']->hasError()):
                                    	    echo $oServiceTypeForm['name']->renderError();
                                        endif;
									    echo $oServiceTypeForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oServiceTypeForm['is_enabled']->renderLabel($oServiceTypeForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oServiceTypeForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oServiceTypeForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
