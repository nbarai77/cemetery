...
class countryActions extends sfActions
{
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_country';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('Country', $request->getParameter('id'),'id');
        ...
        $oGuardGroupPageListQuery = Doctrine::getTable('Country')->getCountryList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('Country', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('Country')->find($snIdGuardGroup));
            $this->oCountryForm = new CountryForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oCountryForm = new CountryForm();
		}

        $this->getConfigurationFields($this->oCountryForm);

        if($oRequest->isMethod('post'))
        {
            $this->oCountryForm->bind($oRequest->getParameter($this->oCountryForm->getName()));

            if($this->oCountryForm->isValid())
            {
                $snIdGroup = $this->oCountryForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('country/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('country/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
