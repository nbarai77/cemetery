...
<div id="wapper">
<?php 
    echo $oCountryForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oCountryForm->getObject()->isNew() ? '?id='.$oCountryForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oCountryForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oCountryForm->getObject()->isNew() ?  __('Add New Country') : __('Edit Country');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oCountryForm['name']->renderLabel($oCountryForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oCountryForm['name']->hasError()):
                                    	    echo $oCountryForm['name']->renderError();
                                        endif;
									    echo $oCountryForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oCountryForm['is_enabled']->renderLabel($oCountryForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oCountryForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oCountryForm->renderHiddenFields(); 
    ?>
    </form>
</div>
