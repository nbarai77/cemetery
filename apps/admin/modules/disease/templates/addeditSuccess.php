...
<div id="wapper">
<?php 
    echo $oDiseaseForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oDiseaseForm->getObject()->isNew() ? '?id='.$oDiseaseForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oDiseaseForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oDiseaseForm->getObject()->isNew() ?  __('Add New Disease') : __('Edit Disease');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oDiseaseForm['name']->renderLabel($oDiseaseForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oDiseaseForm['name']->hasError()):
                                    	    echo $oDiseaseForm['name']->renderError();
                                        endif;
									    echo $oDiseaseForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oDiseaseForm['is_enabled']->renderLabel($oDiseaseForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oDiseaseForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oDiseaseForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
