...
class diseaseActions extends sfActions
{
    /**
     * preExecutes  action
     *
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_disease';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('Disease', $request->getParameter('id'),'id');
            $this->getUser()->setFlash('snSuccessMsgKey', 3);      
        }

        // Get cms page list for listing.
        $oGuardGroupPageListQuery = Doctrine::getTable('Disease')->getDiseaseList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('Disease', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listDiseaseUpdate');
    }
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('Disease')->find($snIdGuardGroup));
            $this->oDiseaseForm = new DiseaseForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oDiseaseForm = new DiseaseForm();
		}

        $this->getConfigurationFields($this->oDiseaseForm);

        if($oRequest->isMethod('post'))
        {
            $this->oDiseaseForm->bind($oRequest->getParameter($this->oDiseaseForm->getName()));

            if($this->oDiseaseForm->isValid())
            {
                $snIdGroup = $this->oDiseaseForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('disease/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('disease/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
