    ...
    */
	public function executePrintLetters(sfWebRequest $oRequest)
	{
		...
		if(count($amResultSet[0]['IntermentBookingFive']) > 0){
			...
			//$oPDF->SetAutoPageBreak(TRUE, PDF_MARGIN_BOTTOM);
			$oPDF->SetFont('helvetica', '', 8, '', true);
			$oPDF->setHeaderTemplateAutoreset(true);


			//HEADER AND FOOTER IMAGE
			$amMailContent = Doctrine::getTable('MailContent')->getMailContentAsPerType($ssContentType,$snCemeteryId);
			//path
			$ssLivePath = "http://admin.cemetery.ntn/js/ckfinder/userfiles/images/";
			$oPDF->ssLivePath = $ssLivePath;
			...
			$ssLetterContent = str_replace($asFinalContent[0],'',$amMailContent[0]['content']);
			...
			//Footer
			$asImageFooter = sfGeneral::getAllStringBetween($amMailContent[0]['content'],'{footer_image}','{/footer_image}');
            if(isset($asImageFooter[0])) {
	            preg_match('/src="([^"]*)"/', $asImageFooter[0], $footermatches);
	            $asFooterImageName = explode('/',$footermatches[1]);
	            $ssFooterImageName = $asFooterImageName[count($asFooterImageName) - 1];
				$oPDF->ssFooterImage = $ssFooterImageName;
			}
			...
			//Header1
			$asImageHeader1 = sfGeneral::getAllStringBetween($amMailContent[0]['content'],'{header_image1}','{/header_image1}');	
			$asStringHeader1 = sfGeneral::getAllStringBetween($amMailContent[0]['content'],'{header_string1}','{/header_string1}');
            if(isset($asImageHeader1[0])) {
	            preg_match('/src="([^"]*)"/', $asImageHeader1[0], $headermatches);
    	        $asFooterImageName1 = explode('/',$headermatches[1]);
    	        $ssHeaderImageName1 = $asFooterImageName1[count($asFooterImageName1) - 1];
			}

			//Header2
			$asImageHeader2 = sfGeneral::getAllStringBetween($amMailContent[0]['content'],'{header_image2}','{/header_image2}');	
			$asStringHeader2 = sfGeneral::getAllStringBetween($amMailContent[0]['content'],'{header_string2}','{/header_string2}');
            if(isset($asImageHeader2[0])) {
	            preg_match('/src="([^"]*)"/', $asImageHeader2[0], $headermatches);
    	        $asFooterImageName2 = explode('/',$headermatches[1]);
    	        $ssHeaderImageName2 = $asFooterImageName2[count($asFooterImageName2) - 1];
			}


			//echo $ssLivePath.$ssHeaderImageName1;exit;
			
			if(isset($ssHeaderImageName1) && $ssHeaderImageName1 != '') {
				$headerImagePath1 = '../../../../../web/js/ckfinder/userfiles/images/'.$ssHeaderImageName1;
				$oPDF->SetHeaderData($headerImagePath1, 130, $asStringHeader2, '');
			}else {
				$oPDF->SetHeaderData("alpha.png", 130, '', '');
			}	
			$oPDF->AddPage();
			$oPDF->writeHTML($ssLetterContent);

			if(isset($ssHeaderImageName2) && $ssHeaderImageName2 != '') {
				$headerImagePath2 = '../../../../../web/js/ckfinder/userfiles/images/'.$ssHeaderImageName2;
				$oPDF->SetHeaderData($headerImagePath2, 130, $asStringHeader2, '');
			}else {
				$oPDF->SetHeaderData("image_demo.jpg", 130, '', '');
			}
    ...
    */
	public function executePrintBurialCertificate(sfWebRequest $request)
	{
		...
		if(count($amIntermentDetails) > 0)
		{
			...
			$oPDF->ssLivePath = "http://admin.cemetery.ntn/js/ckfinder/userfiles/images/";
            $oPDF->ssFooterImage = 'logo.png';
