...
<div id="wapper">
<?php 
    echo $oServiceForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oServiceForm->getObject()->isNew() ? '?id='.$oServiceForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oServiceForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oServiceForm->getObject()->isNew() ?  __('Add New Service') : __('Edit Service');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oServiceForm['name']->renderLabel($oServiceForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oServiceForm['name']->hasError()):
                                    	    echo $oServiceForm['name']->renderError();
                                        endif;
									    echo $oServiceForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oServiceForm['is_enabled']->renderLabel($oServiceForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oServiceForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oServiceForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
