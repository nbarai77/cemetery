...
class serviceActions extends sfActions
{
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_Service';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('FndService', $request->getParameter('id'),'id');
            $this->getUser()->setFlash('snSuccessMsgKey', 3);      
        }

        // Get cms page list for listing.
        $oGuardGroupPageListQuery = Doctrine::getTable('FndService')->getServiceList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('FndService', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listServiceUpdate');
    }
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('FndService')->find($snIdGuardGroup));
            $this->oServiceForm = new FndServiceForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oServiceForm = new FndServiceForm();
		}

        $this->getConfigurationFields($this->oServiceForm);

        if($oRequest->isMethod('post'))
        {
            $this->oServiceForm->bind($oRequest->getParameter($this->oServiceForm->getName()));

            if($this->oServiceForm->isValid())
            {
                $snIdGroup = $this->oServiceForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('service/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('service/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
