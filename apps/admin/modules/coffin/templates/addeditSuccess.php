...
<div id="wapper">
<?php 
    echo $oCoffinForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oCoffinForm->getObject()->isNew() ? '?id='.$oCoffinForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oCoffinForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oCoffinForm->getObject()->isNew() ?  __('Add New Coffin') : __('Edit Coffin');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oCoffinForm['name']->renderLabel($oCoffinForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oCoffinForm['name']->hasError()):
                                    	    echo $oCoffinForm['name']->renderError();
                                        endif;
									    echo $oCoffinForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oCoffinForm['is_enabled']->renderLabel($oCoffinForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oCoffinForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oCoffinForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
