...
class coffinActions extends sfActions
{
    /**
     * preExecutes  action
     *
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_coffin';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('CoffinType', $request->getParameter('id'),'id');
            $this->getUser()->setFlash('snSuccessMsgKey', 3);      
        }

        // Get cms page list for listing.
        $oGuardGroupPageListQuery = Doctrine::getTable('CoffinType')->getCoffinList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('CoffinType', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listCoffinUpdate');
    }
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('CoffinType')->find($snIdGuardGroup));
            $this->oCoffinForm = new CoffinTypeForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oCoffinForm = new CoffinTypeForm();
		}

        $this->getConfigurationFields($this->oCoffinForm);

        if($oRequest->isMethod('post'))
        {
            $this->oCoffinForm->bind($oRequest->getParameter($this->oCoffinForm->getName()));

            if($this->oCoffinForm->isValid())
            {
                $snIdGroup = $this->oCoffinForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('coffin/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('coffin/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
