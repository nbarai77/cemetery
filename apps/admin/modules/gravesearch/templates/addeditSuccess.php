...
<?php 
    echo $oArGraveForm->renderFormTag(
        url_for($sf_params->get('module').'/addedit?'.html_entity_decode($amExtraParameters['ssQuerystr']) ), 
        array("name" => $oArGraveForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo __('Search For Grave');?></h1>
    <div id="message">
        <div id="success_msgs">
            		...
            		<div id="info">
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
								...
								<tr>
									...
									<td valign="middle" width="30%">
										<?php 
											...
											echo $oArGraveForm['country_id']->render(); 
										?>
									</td>
							...
							<tr>
								...
								<td valign="middle" align="right" width="20%">
									<?php echo $oArGraveForm['ar_grave_status_id']->renderLabel($oArGraveForm['ar_grave_status_id']->renderLabelName());?>
                        		</td>
...
echo javascript_tag(
  jq_remote_function(array(
    ...
    'url'     => url_for('gravesearch/getCementryListAsPerCountry?id='.$sf_user->getAttribute('cn').'&cnval='.$sf_user->getAttribute('cm')),
...
if($sf_user->getAttribute('cm') != ''):
	echo javascript_tag(
		jq_remote_function(array(
			'update'  => 'grave_area_list',
			'url'     => url_for('gravesearch/getAreaListAsPerCemetery?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&arval='.$sf_user->getAttribute('ar')),
			'loading' => '$("#IdAjaxLocaderArea").show();',
	...
	if($sf_user->getAttribute('ar') == ''):
		 echo javascript_tag(
		  jq_remote_function(array(
			'update'  => 'grave_section_list',
			'url'     => url_for('gravesearch/getSectionListAsPerArea?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&secval='.$sf_user->getAttribute('sc')),
			'loading' => '$("#IdAjaxLocaderSection").show();',
	...
	if($sf_user->getAttribute('sc') == '' && $sf_user->getAttribute('ar') == ''): 
		echo javascript_tag(
			jq_remote_function(array(
				'update'  => 'grave_row_list',
				'url'     => url_for('gravesearch/getRowListAsPerSection?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&rwval='.$sf_user->getAttribute('rw')),
				'loading' => '$("#IdAjaxLocaderRow").show();',
	...
	if($sf_user->getAttribute('rw') == '' && $sf_user->getAttribute('sc') == '' && $sf_user->getAttribute('ar') == ''): 
	 echo javascript_tag(
		jq_remote_function(array(
			'update'  => 'grave_plot_list',
			'url'     => url_for('gravesearch/getPlotListAsPerRow?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&plval='.$sf_user->getAttribute('pl')),
			'loading' => '$("#IdAjaxLocaderPlot").show();',
	...
	if($sf_user->getAttribute('pl') == '' && $sf_user->getAttribute('rw') == '' && $sf_user->getAttribute('sc') == '' && $sf_user->getAttribute('ar') == ''):
		echo javascript_tag(
		  jq_remote_function(array(
			'update'  => 'grave_list',
			'url'     => url_for('gravesearch/getGraveListAsPerPlot?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&gnval='.$sf_user->getAttribute('gn')),
			'loading' => '$("#IdAjaxLocaderGrave").show();',
		  	'complete'=> '$("#IdAjaxLocaderGrave").hide();'
		  )));
	endif;	  	
...
if($sf_user->getAttribute('ar') != ''): 
	 echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_section_list',
		'url'     => url_for('gravesearch/getSectionListAsPerArea?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&secval='.$sf_user->getAttribute('sc')),
		'loading' => '$("#IdAjaxLocaderSection").show();',
	  	'complete'=> '$("#IdAjaxLocaderSection").hide();'
	  ))); 
	 ...
	 echo javascript_tag(
	  ...
	  jq_remote_function(array(
		'update'  => 'grave_row_list',
		'url'     => url_for('gravesearch/getRowListAsPerSection?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&rwval='.$sf_user->getAttribute('rw')),
		'loading' => '$("#IdAjaxLocaderRow").show();',
	  	'complete'=> '$("#IdAjaxLocaderRow").hide();'
	  ))); 
	 endif; 
	 ...
	 echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_plot_list',
		'url'     => url_for('gravesearch/getPlotListAsPerRow?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&plval='.$sf_user->getAttribute('pl')),
		'loading' => '$("#IdAjaxLocaderPlot").show();',
	  	'complete'=> '$("#IdAjaxLocaderPlot").hide();'
	  ))); 
	 endif;  
	...
	echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_list',
		'url'     => url_for('gravesearch/getGraveListAsPerPlot?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&gnval='.$sf_user->getAttribute('gn')),
		'loading' => '$("#IdAjaxLocaderGrave").show();',
  		'complete'=> '$("#IdAjaxLocaderGrave").hide();'
	  )));
	endif;	 
 ...
 if($sf_user->getAttribute('sc') != ''): 
	 echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_row_list',
		'url'     => url_for('gravesearch/getRowListAsPerSection?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&section_id='.$sf_user->getAttribute('sc').'&rwval='.$sf_user->getAttribute('rw')),
		'loading' => '$("#IdAjaxLocaderRow").show();',
	  	'complete'=> '$("#IdAjaxLocaderRow").hide();'
	  ))); 
	 ...
	 echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_plot_list',
		'url'     => url_for('gravesearch/getPlotListAsPerRow?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&section_id='.$sf_user->getAttribute('sc').'&plval='.$sf_user->getAttribute('pl')),
		'loading' => '$("#IdAjaxLocaderPlot").show();',
	  	'complete'=> '$("#IdAjaxLocaderPlot").hide();'
	  ))); 
	 endif;
	...
	echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_list',
		'url'     => url_for('gravesearch/getGraveListAsPerPlot?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&section_id='.$sf_user->getAttribute('sc').'&gnval='.$sf_user->getAttribute('gn')),
		'loading' => '$("#IdAjaxLocaderGrave").show();',
	  	'complete'=> '$("#IdAjaxLocaderGrave").hide();'
	  )));
	endif;	 
 ...
 if($sf_user->getAttribute('rw') != ''): 
	 echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_plot_list',
		'url'     => url_for('gravesearch/getPlotListAsPerRow?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&section_id='.$sf_user->getAttribute('sc').'&row_id='.$sf_user->getAttribute('rw').'&plval='.$sf_user->getAttribute('pl')),
		'loading' => '$("#IdAjaxLocaderPlot").show();',
	  	'complete'=> '$("#IdAjaxLocaderPlot").hide();'
	  )));
	...
	echo javascript_tag(
	  jq_remote_function(array(
		'update'  => 'grave_list',
		'url'     => url_for('gravesearch/getGraveListAsPerPlot?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&section_id='.$sf_user->getAttribute('sc').'&row_id='.$sf_user->getAttribute('rw').'&gnval='.$sf_user->getAttribute('gn')),
		'loading' => '$("#IdAjaxLocaderGrave").show();',
	  	'complete'=> '$("#IdAjaxLocaderGrave").hide();'
	  )));
	endif;   
...
echo javascript_tag(
  jq_remote_function(array(
    'update'  => 'grave_list',
    'url'     => url_for('gravesearch/getGraveListAsPerPlot?country_id='.$sf_user->getAttribute('cn').'&cemetery_id='.$sf_user->getAttribute('cm').'&area_id='.$sf_user->getAttribute('ar').'&section_id='.$sf_user->getAttribute('sc').'&row_id='.$sf_user->getAttribute('rw').'&plot_id='.$sf_user->getAttribute('pl').'&gnval='.$sf_user->getAttribute('gn')),
	'loading' => '$("#IdAjaxLocaderGrave").show();',
	'complete'=> '$("#IdAjaxLocaderGrave").hide();'
  )));
endif;
    ...
    echo javascript_tag("
        jQuery(document).ready(function() 
		{
			...
			if(snCountryId > 0 && snCemeteryId == 1)
				callAjaxRequest(snCountryId,'".url_for('gravesearch/getCementryListAsPerCountry')."','grave_cemetery_list');
		});
    ");
endif;
echo javascript_tag("
