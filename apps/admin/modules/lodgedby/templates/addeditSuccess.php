...
<div id="wapper">
<?php 
    echo $oLodgedByForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oLodgedByForm->getObject()->isNew() ? '?id='.$oLodgedByForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oLodgedByForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oLodgedByForm->getObject()->isNew() ?  __('Add New Lodged By') : __('Edit Lodged By');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oLodgedByForm['name']->renderLabel($oLodgedByForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oLodgedByForm['name']->hasError()):
                                    	    echo $oLodgedByForm['name']->renderError();
                                        endif;
									    echo $oLodgedByForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oLodgedByForm['is_enabled']->renderLabel($oLodgedByForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oLodgedByForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oLodgedByForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
