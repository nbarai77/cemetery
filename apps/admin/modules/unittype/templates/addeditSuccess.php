...
<div id="wapper">
<?php 
    echo $oUnitTypeForm->renderFormTag(
        url_for($sf_params->get('module').'/'.$sf_params->get('action').(!$oUnitTypeForm->getObject()->isNew() ? '?id='.$oUnitTypeForm->getObject()->getId().'&'.html_entity_decode($amExtraParameters['ssQuerystr']) : '')), 
        array("name" => $oUnitTypeForm->getName(), "method" => "post" ,'enctype'=>'multipart/form-data')
    );
?>
    <div class="comment_list"></div>
    <h1><?php echo $oUnitTypeForm->getObject()->isNew() ?  __('Add New Unit Type') : __('Edit Unit Type');?></h1>
    <div id="message">
        <div id="success_msgs">
                		...
                		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="sub_table">
                    		<tr>
                            	<td valign="middle" align="right" width="20%">
                                	<?php echo $oUnitTypeForm['name']->renderLabel($oUnitTypeForm['name']->renderLabelName()."<span class='redText'>*</span>");?>
                        		</td>
	
                            	<td valign="middle" width="80%">
                                	<?php 
                                	    if($oUnitTypeForm['name']->hasError()):
                                    	    echo $oUnitTypeForm['name']->renderError();
                                        endif;
									    echo $oUnitTypeForm['name']->render(array('class'=>'inputBoxWidth')); 
								    ?>
                                </td>
                    		</tr>
                        <tr>
                            <td valign="middle">
                                <?php echo $oUnitTypeForm['is_enabled']->renderLabel($oUnitTypeForm['is_enabled']->renderLabelName());?>
                            </td>
                            <td valign="middle" colspan="3"> <?php echo $oUnitTypeForm['is_enabled']->render();?> </td>
                        </tr>
                    		<tr>
                            	<td>&nbsp;</td>
                        		<td valign="middle">
    ...
    <?php
        ...
        echo $oUnitTypeForm->renderHiddenFields(); 
    ?>
    </form>
</div>
<?php 
