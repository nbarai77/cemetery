...
class unitTypeActions extends sfActions
{
    /**
     * preExecutes  action
     *
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_unit_type';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('UnitType', $request->getParameter('id'),'id');
            $this->getUser()->setFlash('snSuccessMsgKey', 3);      
        }

        // Get cms page list for listing.
        $oGuardGroupPageListQuery = Doctrine::getTable('UnitType')->getUnitTypeList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('UnitType', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listUnitTypeUpdate');
    }
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('UnitType')->find($snIdGuardGroup));
            $this->oUnitTypeForm = new UnitTypeForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oUnitTypeForm = new UnitTypeForm();
		}

        $this->getConfigurationFields($this->oUnitTypeForm);

        if($oRequest->isMethod('post'))
        {
            $this->oUnitTypeForm->bind($oRequest->getParameter($this->oUnitTypeForm->getName()));

            if($this->oUnitTypeForm->isValid())
            {
                $snIdGroup = $this->oUnitTypeForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('unittype/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('unittype/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
