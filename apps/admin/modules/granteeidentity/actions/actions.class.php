...
/**
 * granteeidentity actions.
 *
 * @package    cemetery
 * @subpackage granteeidentity
 * @author     Your name here
 * @version    SVN: $Id: actions.class.php,v 1.1.1.1 2012/03/24 12:17:19 nitin Exp $
 */
 sfProjectConfiguration::getActive()->loadHelpers(array('I18N'));
class granteeidentityActions extends sfActions
{
    /**
     * preExecutes  action
     *
    ...
    public function preExecute()
    {
        ...
        $this->ssFormName = 'frm_list_grantee_identity';
    ...
    */
    public function executeIndex(sfWebRequest $request)
    {
        ...
        {
            $omCommon->DeleteRecordsComposite('GranteeIdentity', $request->getParameter('id'),'id');
        ...
        {   
            ...
            $omCommon->UpdateStatusComposite('sfGuardUser','is_active', $request->getParameter('id'), $ssStatus, 'id');
        ...
        $oGuardGroupPageListQuery = Doctrine::getTable('GranteeIdentity')->getGranteeIdentityList($this->amExtraParameters, $this->amSearch);

        // Set pager and get results
        $oPager               = new sfMyPager();
        $this->oGuardGroupList  = $oPager->getResults('GranteeIdentity', $this->snPaging,$oGuardGroupPageListQuery,$this->snPage);
        ...
        if($request->getParameter('request_type') == 'ajax_request')
            $this->setTemplate('listCountryUpdate');
    }
    /**
    * update action
    *
    ...
    */
    public function executeAddedit(sfWebRequest $oRequest)
    {
        ...
        if($snIdGuardGroup)
        {
            $this->forward404Unless($oGroup = Doctrine::getTable('GranteeIdentity')->find($snIdGuardGroup));
            $this->oGranteeIdentityForm = new GranteeIdentityForm($oGroup);
            $ssSuccessKey = 2; // Success message key for edit
        }
        else {
            $this->oGranteeIdentityForm = new GranteeIdentityForm();
		}

        $this->getConfigurationFields($this->oGranteeIdentityForm);

        if($oRequest->isMethod('post'))
        {
            $this->oGranteeIdentityForm->bind($oRequest->getParameter($this->oGranteeIdentityForm->getName()));

            if($this->oGranteeIdentityForm->isValid())
            {
                $snIdGroup = $this->oGranteeIdentityForm->save()->getId();

                $this->getUser()->setFlash('snSuccessMsgKey', $ssSuccessKey);   //Set messages for add and update records

                if($oRequest->getParameter('tab'))
                    $this->redirect('granteeidentity/addedit?id='.$snIdGroup.'&tab='.$oRequest->getParameter('tab').'&'.$this->amExtraParameters['ssQuerystr']);
                else
                	$this->redirect('granteeidentity/index?'.$this->amExtraParameters['ssQuerystr']);
            }
        }
    }
    /**
