...
{ 
    ...
    /*
	var ssCurrentPage = parseInt(jQuery('#page').val()) - 1;
    jQuery('#page').val(ssCurrentPage);
...
function showHideDiv(ssType,ssDivId,ssValue)
{
	if(ssType == "chapel" && ssValue == "YES")
		$("#"+ssDivId).show();
	else if(ssType == "room" && ssValue == "YES")
		$("#"+ssDivId).show();
	else if(ssType == "facility" && ssValue == "YES")
		$("#"+ssDivId).show();
	else
		$("#"+ssDivId).hide();
...
function callAjaxForCemetery(snCemeteryId, snIdCountry, snGranteeId)
{
	/*
	var amUpdateDivAndActions = ssUpdateDiv.split('|');
	for(var snI = 0; snI < amUpdateDivAndActions.length; snI++)
	{
		var amActionAndDiv = amUpdateDivAndActions[snI].split('::');
		var ssUpdateDiv = amActionAndDiv[0];
		var ssAction = amActionAndDiv[1];
		
		jQuery.ajax({
			type:'POST',
			dataType:'html',
			data:{country_id: snIdCountry,cemetery_id: snCemeteryId},
			success:function(data, extStatus){jQuery("#"+ssUpdateDiv).html(data);},
			loading:function(XMLHttpRequest){$("#indicatormedia").show();},
			complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
			url:'/'+ssModuleName+'/'+ssAction
		});
	}
	*/
	
	// For get area as per country and cemetery
	
	jQuery.ajax({
			type:'POST',
			dataType:'html',
			data:{country_id: snIdCountry,cemetery_id: snCemeteryId},
			success:function(data, extStatus){jQuery("#grantee_area_list").html(data);},
			loading:function(XMLHttpRequest){$("#indicatormedia").show();},
			complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
			url:'/grantee/getAreaListAsPerCemetery'
		});
	
	// For get section as per country and cemetery
	jQuery.ajax({
		type:'POST',
		dataType:'html',
		data:{country_id: snIdCountry,cemetery_id: snCemeteryId},
		success:function(data, extStatus){jQuery("#grantee_section_list").html(data);},
		beforeSend:function(XMLHttpRequest){$("#indicatormedia").show();},
		complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
		url:'/grantee/getSectionListAsPerArea'
	});
	// For get row as per country and cemetery
	jQuery.ajax({
		type:'POST',
		dataType:'html',
		data:{country_id: snIdCountry,cemetery_id: snCemeteryId},
		success:function(data, extStatus){jQuery("#grantee_row_list").html(data);},
		beforeSend:function(XMLHttpRequest){$("#indicatormedia").show();},
		complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
		url:'/grantee/getRowListAsPerSection'
	});
	// For get plot as per country and cemetery
	jQuery.ajax({
		type:'POST',
		dataType:'html',
		data:{country_id: snIdCountry,cemetery_id: snCemeteryId},
		success:function(data, extStatus){jQuery("#grantee_plot_list").html(data);},
		beforeSend:function(XMLHttpRequest){$("#indicatormedia").show();},
		complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
		url:'/grantee/getPlotListAsPerRow'
	});
	// For get grave as per country and cemetery
	jQuery.ajax({
		type:'POST',
		dataType:'html',
		data:{country_id: snIdCountry,cemetery_id: snCemeteryId,grantee_id: snGranteeId},
		success:function(data, extStatus){jQuery("#grantee_grave_list").html(data);},
		beforeSend:function(XMLHttpRequest){$("#indicatormedia").show();},
		complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
		url:'/grantee/getGraveListAsPerPlot'
	});
	// For get funeral directors as per country and cemetery
	jQuery.ajax({
		type:'POST',
		dataType:'html',
		data:{cemetery_id: snCemeteryId},
		success:function(data, extStatus){jQuery("#grantee_funeral_list").html(data);},
		beforeSend:function(XMLHttpRequest){$("#indicatormedia").show();},
		complete:function(XMLHttpRequest, textStatus){$("#indicatormedia").hide();},
		url:'/grantee/getFuneralListAsPerCemetery'
	});
